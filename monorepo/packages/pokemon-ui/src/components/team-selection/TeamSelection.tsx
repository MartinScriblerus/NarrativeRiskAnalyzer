...
export const TopicSelection = () => {
  ...
  // local UI state for the selected topic's pokemon names
  const [topicPokemonNames, setTopicPokemonNames] = useState<string[]>([]);
  ...
  const showSelectedTopicPokemons = useCallback(
    async (topic: any) => {
      if (!topic?.id) return;
      ...
      try {
        ...
        setTopicPokemonNames((namesRes.data?.pokemonNames ?? []).slice(0, 6));
      } catch (err) {
        console.warn('Could not fetch pokemon names for topic', err);
        setTopicPokemonNames([]);
      }
    },
  ...
  useEffect(() => {
    ...
    if (topicObj) {
      ...
      showSelectedTopicPokemons(topicObj);
    } else {
      // topics not loaded yet or topic not present -> call handler with minimal object (it will still fetch names)
      showSelectedTopicPokemons({ id: topicIdFromState });
    }
  }, [topics, topicIdFromState, selectedPortfolio, setSelectedPortfolio, showSelectedTopicPokemons]);
  ...
  const handleTopicCreated = (createdTopic: any) => {
    ...
    // if this topic belongs to the currently-selected profile, show its pokemons and highlight
    if (topicProfileId && topicProfileId === effectiveProfileId) {
      showSelectedTopicPokemons(createdTopic);
      ...
      <Section direction="column" gap="0px" padding="0">
        <Panel background="primary" rounded={false} padding="0">
          <Stack direction="row">
            ...
            <CurrProfileColumn>
              <Typography as="h3" variant="body" color="#000">
                Current Profile:
              </Typography>
            ...
            <Column>
              ...
              <Stack background="primaryPokemon" direction="column" spacing="4px" scroll="vertical" maxHeight={100} paddingTop={4}>
                {displayedTopics.map((t) => {
                  ...
                  return (
                    <Button key={t.id} onClick={() => showSelectedTopicPokemons(t)} background={selectedTopicId === t.id ? 'accent' : 'secondary'} type="button">
                      {t.name}
                    </Button>
                  );
                })}
            ...
            {/* Column 3: Pokemon on selected topic (grid up to 6 items) */}
            <Column>
              <Typography as="h3" variant="body" color="#000">
                Selected Topic
              </Typography>
              <Grid columns="repeat(auto-fit, minmax(120px, 1fr))" gap="8px">
                {Array.from({ length: 6 }).map((_, idx) => {
                  const name = topicPokemonNames[idx];
                  return name ? (
